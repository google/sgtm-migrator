...
const bootstrap = lazy(() => {
  ...
  const accountIdWeb = gtm_web_url.match(/\d+/g)[0];
  const containerIdWeb = gtm_web_url.match(/\d+/g)[1];
  const workspaceIdWeb = gtm_web_url.match(/\d+/g)[2];
  const accountIdServer = gtm_server_url.match(/\d+/g)[0];
  const containerIdServer = gtm_server_url.match(/\d+/g)[1];
  const workspaceIdServer = gtm_server_url.match(/\d+/g)[2];
